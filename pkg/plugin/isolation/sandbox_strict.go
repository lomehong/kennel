...
func (s *StrictIsolationSandbox) Stop() error {
	...
	if s.cmd != nil && s.cmd.Process != nil {
		s.logger.Info("停止子进程", "id", s.id, "pid", s.cmd.Process.Pid)
		if err := s.cmd.Process.Kill(); err != nil {
			s.logger.Error("停止子进程失败", "id", s.id, "pid", s.cmd.Process.Pid, "error", err)
		}
	}
