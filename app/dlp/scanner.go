...
// 辅助函数，用于从配置中获取布尔值
func getConfigBoolFromScanner(config map[string]interface{}, key string, defaultValue bool) bool {
...
// 辅助函数，用于从配置中获取字符串切片
func getConfigStringSliceFromScanner(config map[string]interface{}, key string) []string {
	if val, ok := config[key]; ok {
...
func NewScanner(logger logging.Logger, ruleManager *RuleManager, alertManager *AlertManager, config map[string]interface{}) *Scanner {
	return &Scanner{
		logger:       logger,
		ruleManager:  ruleManager,
		alertManager: alertManager,
...
func (s *Scanner) ScanDirectory(dir string) ([]DLPAlert, error) {
	...
	// 获取监控的文件类型
	fileTypes := getConfigStringSliceFromScanner(s.config, "monitored_file_types")
	if len(fileTypes) == 0 {
...
func (s *Scanner) MonitorClipboard() error {
	...
	// 检查是否启用剪贴板监控
	if !getConfigBoolFromScanner(s.config, "monitor_clipboard", false) {
...
func (s *Scanner) MonitorFiles() error {
	...
	// 检查是否启用文件监控
	if !getConfigBoolFromScanner(s.config, "monitor_files", false) {
		s.logger.Info("文件监控已禁用")
		return nil
	}

	// 获取监控的目录
	dirs := getConfigStringSliceFromScanner(s.config, "monitored_directories")
	if len(dirs) == 0 {
